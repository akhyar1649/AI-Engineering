# Assignment — Scheduling & Triggering

## Konteks kasus

Kirimin memiliki real-time status update, nightly reconciliation, dan daily backlog report.

## Tugas dan requirement

1. Buat trigger matrix dan timezone policy.
2. Implementasikan mock webhook/schedule/poll trigger.
3. Definisikan logical run key, dedup, missed run, dan backfill.
4. Buat concurrency/rate limit policy.
5. Uji duplicate, late event, rerun, dan daylight/timezone case.

## Kriteria penilaian

- 25% trigger fit.
- 25% temporal correctness.
- 20% idempotency/backfill.
- 20% rate/concurrency.
- 10% tests.
