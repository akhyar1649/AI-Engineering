...
Ops Lead Sari ingin SLA pickup-to-delivery per kota dan daftar order yang memiliki status mundur atau event tidak berurutan.

## Tugas dan requirement

1. Buat CTE untuk memilih event terbaru per order.
2. Hitung durasi antar status memakai `LAG`.
3. Hitung median atau percentile dengan pendekatan yang tersedia pada database target dan jelaskan keterbatasannya.
4. Deteksi transisi status invalid dan timestamp out-of-order.
5. Sertakan contoh `EXPLAIN` dan dua hipotesis optimasi.

## Bahan/dataset

Minimal 200 event sintetis, mencakup satu order dengan duplicate event dan satu event out-of-order.

## Kriteria penilaian

- 30% correctness window/CTE.
- 25% perhitungan waktu dan NULL.
- 20% deteksi anomali.
- 15% reasoning performa.
- 10% dokumentasi grain.
