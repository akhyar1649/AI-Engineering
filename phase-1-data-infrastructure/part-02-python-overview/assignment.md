# Assignment — Python Overview

## Konteks kasus

Kirimin menerima batch order CSV dari web, mobile, dan partner API. Anda perlu membuat fungsi transformasi yang dapat dipanggil oleh scheduler tanpa mengubah input asli.

## Tugas dan requirement

1. Buat `transform_orders(frame)` dengan kontrak kolom wajib dan return `clean_frame, quality_report`.
2. Standardisasi kota minimal untuk `Jakarta`, `JKT`, `jakarta`, dan spasi berlebih.
3. Konversi nilai order menjadi numeric; baris invalid tidak boleh hilang tanpa dicatat.
4. Deduplikasi berdasarkan `order_id` menggunakan aturan yang Anda dokumentasikan.
5. Tambahkan unit test untuk input normal, kolom hilang, duplicate, dan nilai invalid.

## Bahan/dataset

Dataset sintetis dari notebook atau generator Anda sendiri, minimal 100 baris dengan missing value dan schema variation.

## Kriteria penilaian

- 30% kontrak fungsi dan validasi.
- 25% transformasi pandas.
- 20% penanganan error dan quality report.
- 15% testability.
- 10% dokumentasi.
