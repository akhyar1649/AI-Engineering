# Assignment — NoSQL in Practice (MongoDB)

## Konteks kasus

Buat ingestion service kecil untuk courier tracking events dengan duplicate retry dan event terlambat.

## Tugas dan requirement

1. Implementasikan collection schema dan validation rule.
2. Buat unique event key dan ingestion idempotent.
3. Tambahkan index untuk latest status dan pencarian per hub/time.
4. Buat aggregation latest status per shipment.
5. Dokumentasikan retention/TTL, backup, dan failure behavior.

## Kriteria penilaian

- 25% schema/access pattern.
- 25% idempotency.
- 20% index/query.
- 15% aggregation.
- 15% reliability/retention.
